...
def dealWithCallMInstr(lab,instr,toLab):
    ...
    if entryP != "":#self-definition method
        ...
        while paraList.find("type_")!=-1:
            ...
            fact = fact+";defF("+lab+","+paraList[:paraList.find(")")+1]+")"
...
prolog.assertz("purepath(X,Y,V):-purepath(X,T,V),flow(T,Y),\+defF(Y,V),X \== Y")
...
#     print(key+dataDepStack[key])
    while dataDepStack[key].find(";") != -1:
        count = count+1
        dataDepFact = "dataDepF("+key+","+dataDepStack[key].split(";",1)[0].split("^")[0]+","+dataDepStack[key].split(";",1)[0].split("^")[1]+")"
